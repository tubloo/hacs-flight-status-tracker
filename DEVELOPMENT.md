...
- **Sensor**: `sensor.py` keeps the canonical unified flight list in-memory (`hass.data`), exposes a summary sensor (`sensor.flight_status_tracker_upcoming_flights`), and creates/removes dynamic per-flight sensor entities keyed by `flight_key`.
